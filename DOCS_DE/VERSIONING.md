...
Diese Datei definiert die verbindlichen Anforderungen, Abläufe, Zuständigkeiten und Qualitätsmaßstäbe für den Bereich **Versionierungsrichtlinie** innerhalb des BylickiLabs Web Security Inspector.
...
Das Projekt verwendet ein an Semantic Versioning angelehntes Schema:

```text
MAJOR.MINOR.PATCH
```

- **MAJOR:** inkompatible Architektur- oder Funktionsänderungen
- **MINOR:** abwärtskompatible Funktionen und größere Erweiterungen
- **PATCH:** Fehlerkorrekturen, Sicherheitsupdates und kleinere Optimierungen
