...
Diese Datei definiert die verbindlichen Anforderungen, Abläufe, Zuständigkeiten und Qualitätsmaßstäbe für den Bereich **Scanner-Module** innerhalb des BylickiLabs Web Security Inspector.
...
- **Http-header:** wird verbindlich beschrieben, fachlich eingeordnet und regelmäßig überprüft.
- **Cookies und cors:** wird verbindlich beschrieben, fachlich eingeordnet und regelmäßig überprüft.
- **Formulare und inhalte:** wird verbindlich beschrieben, fachlich eingeordnet und regelmäßig überprüft.
- **Tls und zertifikate:** wird verbindlich beschrieben, fachlich eingeordnet und regelmäßig überprüft.
- **Dns, mail-sicherheit und performance:** wird verbindlich beschrieben, fachlich eingeordnet und regelmäßig überprüft.
...
| Modul | Aufgabe |
|---|---|
| `http_headers.py` | Sicherheitsheader und Antwortinformationen |
| `cookies.py` | Cookie-Attribute |
| `cors.py` | Cross-Origin-Konfiguration |
| `forms.py` | Formulare und Übertragungswege |
| `content.py` | HTML-Inhalte und Ressourcen |
| `tls_scan.py` | TLS und Zertifikate |
| `dns_scan.py` | DNS und Mail-Sicherheit |
| `performance.py` | Antwortzeitmessungen |
| `resources.py` | öffentliche Standard- und Deployment-Dateien |
| `external.py` | optionale externe Dienste |
